...
function _generalised_lombscargle!(P, freqs, times, y, w, Y, YY, nil)
    @inbounds Threads.@threads for n in eachindex(freqs)
        ...
        SS_τ   = 1 - CC_τ - S_τ*S_τ
        CC_τ  -= C_τ*C_τ
        P[n] = sign(CC_τ)*(YC_τ)/sqrt(abs(2*CC_τ)) + 1.0im*sign(SS_τ)*(YS_τ)/sqrt(abs(2*SS_τ))
    end
    return P
end
...
function _generalised_lombscargle!(P, freqs, times, y, w, YY, nil)
    @inbounds Threads.@threads for n in eachindex(freqs)
        ...
        frac_C = ifelse(iszero(CC_τ), nil, sign(CC_τ)*(YC_τ) / sqrt(abs(2*CC_τ)))
        frac_S = ifelse(CC_τ == 1,    nil, sign(1 - CC_τ)*(YS_τ) / sqrt(abs(2*(1 - CC_τ))))
...
_periodogram!(p::GLSPlan_fit_mean) =
    _generalised_lombscargle!(p.P, p.freq, p.times, p.y, p.w, p.Y, p.YY, zero(eltype(real.(p.P))))
_periodogram!(times, p::GLSPlan_fit_mean) =
    _generalised_lombscargle!(p.P, p.freq,   times, p.y, p.w, p.Y, p.YY, zero(eltype(real.(p.P))))
_periodogram!(p::GLSPlan) =
    _generalised_lombscargle!(p.P, p.freq, p.times, p.y, p.w, p.YY, zero(eltype(real.(p.P))))
_periodogram!(times, p::GLSPlan) =
    _generalised_lombscargle!(p.P, p.freq,   times, p.y, p.w, p.YY, zero(eltype(real.(p.P))))
